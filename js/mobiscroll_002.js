    ...
    function Scroller(elem, settings) {
        ...
        */
        that.show = function (prevAnim) {
            ...
            html += (s.display != 'inline' ? '<div class="dwbc' + (s.button3 ? ' dwbc-p' : '') + '" style=" height:36px;overflow:hidden">' + '<span class="dwbw dwb-c"><span class="dwb">' + s.cancelText + '</span></span>' + (s.button3 ? '<span class="dwbw dwb-n"><span class="dwb">' + s.button3Text + '</span></span>' : '') + '<span class="dwbw dwb-s"><div class="opcityBtn">确定</div><span class="dwb" style="display:block; height:36px; line-height:36px; ">' + s.setText + '</span></span>' + '</div></div>' : '<div class="dwcc"></div>') + '</div></div></div>';
            dw = $(html);
